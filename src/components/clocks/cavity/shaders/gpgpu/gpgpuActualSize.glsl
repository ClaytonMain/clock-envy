...
void main() {
  ...
  float positionZ = float(
    int(floor(float(index) / float(uCubeCounts.x))) % int(uCubeCounts.z)
  );

  vec3 position = vec3(positionX, positionY, positionZ);
  position = position / float(uCubeCounts.x);
  vec3 flowField = vec3(
    simplexNoise4d(vec4(position.xyz + uNoiseOffsets.x, uTime)),
    simplexNoise4d(vec4(position.xyz + uNoiseOffsets.y, uTime)),
    simplexNoise4d(vec4(position.xyz + uNoiseOffsets.z, uTime))
  );
  flowField = normalize(flowField);

  vec4 targetSizeInfo = vec4(step(uThreshold, smoothstep(-1.0, 1.0, flowField)), 1.0);
  vec4 actualSizeInfo = texture(actualSizeTexture, uv);

  actualSizeInfo = mix(
    actualSizeInfo,
    targetSizeInfo,
    1.0 - pow(uSizeSpeed, uDelta)
  );
