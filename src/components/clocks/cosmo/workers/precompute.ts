function precomputeWorker() {
  ...
  function getEForDeflectionTexelU(texelU: number): number {
    if (texelU <= 0.5) {
      return Math.sqrt(
        MU * (1.0 - Math.exp(-50.0 * Math.pow(texelU - 0.5, 2))),
      );
    }
    return Math.sqrt(MU / (1.0 - Math.exp(-50.0 * Math.pow(texelU - 0.5, 2))));
  }

  function getDeflectionTableTextureDeu(size: number = DEFLECTION_TABLE_SIZE) {
    const eps = 1e-5;
    const data = new Float32Array(size * size * 2);
    for (let i = 0; i < size; i++) {
      const e = getEForDeflectionTexelU(i / (size - 1));
      let t = 0;
      let u = 0;
      let uDot = e;
      let phi = 0;
      ...
      while (true) {
        ...
        j = getDeuTexelCoordinates(e, u)[1] * (size - 1);
  ...
  function getRayInverseRadiusTableTextureUephi(
    size: number = RAY_INVERSE_RADIUS_TABLE_SIZE,
  ) {
    const eps = 1e-5;
    const data = new Float32Array(size * size * 2);
    ...
    for (let i = 0; i < size; i++) {
      ...
      const clampedTexelU = Math.min(Math.max(i / (size - 1), 0.001), 0.999);
      ...
      while (true) {
        const j = (phi / phiUpperBound) * (size - 1);
  ...
  onmessage = (event) => {
    const { deflectionTableSize, rayInverseRadiusTableSize } = event.data;
    postMessage(
      JSON.parse(
        JSON.stringify({
          type: "precomputing",
          message: "Precomputing textures...",
        }),
      ),
    );
    const deflectionTableTextureData =
      getDeflectionTableTextureDeu(deflectionTableSize);
    const rayInverseRadiusTableTextureData =
      getRayInverseRadiusTableTextureUephi(rayInverseRadiusTableSize);
    postMessage(
      JSON.parse(
        JSON.stringify({
          type: "precomputed",
...
let code = precomputeWorker.toString();
code = code.substring(code.indexOf("{") + 1, code.lastIndexOf("}"));
const blob = new Blob([code], { type: "application/javascript" });
const precomputeWorkerBlobUrl = URL.createObjectURL(blob);
export default precomputeWorkerBlobUrl;
